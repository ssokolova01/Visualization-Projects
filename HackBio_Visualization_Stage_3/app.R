...
cell_type_choices <- c("All", sort(unique(meta_df$cell_type)))
...
server <- function(input, output, session) {
  ...
  output$umap_plot <- renderPlot({
    if (input$cell_type == "All") {
      ...
      ggplot(umap_meta, aes(x = UMAP_1, y = UMAP_2, colour = cell_type)) +
        ...
        labs(
          title    = "All cells - coloured by cell type",
  ...
  output$summary_table <- renderTable({
    summary_rows <- lapply(sort(unique(meta_df$cell_type)), function(ct) {
